<h1 align="center">Septian Dimas Prasetya</h1>
<p align="center"><strong>IT Lead | Systems Builder</strong></p>
<p align="center">Building internal systems, business operations platforms, and e-commerce/logistics software.</p>

<p align="center">
  <a href="https://github.com/Septi-DmsDev">GitHub</a> ·
  <a href="https://septidev.web.id/">Website</a>
</p>

---

## About

I work as an IT Lead in the printing industry, building software that supports real business execution across internal systems, cross-team workflows, commerce operations, and logistics processes.

My role combines technical leadership with hands-on delivery: leading teams, shaping implementation direction, connecting systems across divisions, migrating platforms, and building products from zero into usable internal and business-facing platforms.

---

## Focus

- Internal systems
- Business operations
- E-commerce workflows
- Logistics tooling
- Workflow-driven platforms

---

## Selected Projects

### [HRIS-Internal](https://github.com/Septi-DmsDev/HRIS-Internal)
Internal HRIS platform covering employee data, attendance, performance workflows, overtime, approvals, and payroll-related processes.

**Tech:** `Next.js` `TypeScript` `Supabase` `PostgreSQL` `Drizzle ORM`

### [Ticketing-IT-Teknos](https://github.com/Septi-DmsDev/Ticketing-IT-Teknos)
IT support and request platform with public tracking and protected admin workflows.

**Tech:** `Astro` `TypeScript` `Supabase` `Tailwind CSS`

### [Scanner-Router](https://github.com/Septi-DmsDev/Scanner-Router)
Operational scanner routing application with authentication, dashboard pages, API routes, and logging workflows.

**Tech:** `Next.js` `TypeScript` `SQLite`

### [WPI-INTERNASIONAL](https://github.com/Septi-DmsDev/WPI-INTERNASIONAL)
Company profile website for a packaging and offset manufacturing business, built around process clarity, capabilities, and presentation.

**Tech:** `Next.js` `TypeScript` `GSAP` `Framer Motion` `React Three Fiber`

---

## Experience Highlights

- Leading teams while staying directly involved in implementation
- Building systems used across multiple divisions and operational workflows
- Migrating platforms toward more maintainable architecture and delivery flow
- Building products from zero into usable internal and business-facing systems

---

## Private Platform Scope

In private repositories, my work also covers commerce infrastructure, logistics services, omnichannel support operations, SaaS-style workflow systems, and cross-team internal tooling.

I keep this section intentionally high level to respect private project boundaries while still reflecting the actual scope of my work.

---

## Core Stack

`TypeScript` `Next.js` `React` `Node.js` `PostgreSQL` `Supabase` `Prisma` `Drizzle ORM` `Tailwind CSS` `Astro`

---

## Current Direction

- Building systems that improve operational clarity and execution quality
- Designing around real business constraints, not abstract features
- Raising product quality without losing maintainability
- Staying hands-on while leading technical direction and delivery

